...
const convexSource = (file: string) => readFileSync(resolve(process.cwd(), "convex", file), "utf8");

describe("security boundaries", () => {
  test("alias relationship lookups are internal-only", () => {
    const source = convexSource("aliases.ts");

    expect(source).toMatch(/export const resolveCanonicalMemberId = internalQuery\s*\(/);
