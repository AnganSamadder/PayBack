...
final class FirestoreAccountService: AccountService {
    ...
    func syncFriends(accountEmail: String, friends: [AccountFriend]) async throws {
        do {
            ...
            let accountDocument = try accountDocumentReference(for: accountEmail)
            let friendDocs = accountDocument.collection("friends")
    ...
    func fetchFriends(accountEmail: String) async throws -> [AccountFriend] {
        do {
            ...
            let accountDocument = try accountDocumentReference(for: accountEmail)
            let snapshot = try await accountDocument
                .collection("friends")
                .getDocuments()
    ...
    private func accountDocumentReference(for rawEmail: String) throws -> DocumentReference {
        let sanitizedEmail = try normalizedEmail(from: rawEmail)
        return database.collection(collectionName).document(sanitizedEmail)
    }
