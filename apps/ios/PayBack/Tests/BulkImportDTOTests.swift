...
final class BulkImportDTOTests: XCTestCase {
    ...
    func testBulkExpenseDTO_Encoding() throws {
        let split = BulkSplitDTO(member_id: "member-1", amount: 10.0, percentage: 50.0)
        let subexpense = BulkSubexpenseDTO(description: "Coffee", member_id: "member-1", amount: 5.0)
        ...
        XCTAssertEqual(splits[0]["percentage"] as? Double, 50.0)
        
        let subexpenses = try XCTUnwrap(json["subexpenses"] as? [[String: Any]])
        XCTAssertEqual(subexpenses[0]["description"] as? String, "Coffee")
        XCTAssertEqual(subexpenses[0]["member_id"] as? String, "member-1")
    ...
    func testBulkImportRequest_Encoding() throws {
        ...
        let expense = BulkExpenseDTO(id: "e1", group_id: "g1", description: "D", date: 0, total_amount: 0, paid_by_member_id: "m1", splits: [], subexpenses: [], is_settled: true)
