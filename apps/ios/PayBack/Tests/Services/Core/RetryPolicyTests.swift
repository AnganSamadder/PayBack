...
final class RetryPolicyTests: XCTestCase {
    ...
    // MARK: - Additional Firestore Error Tests
    
    func testExecute_firestoreUnavailable_isRetryable() async {
        let policy = RetryPolicy(maxAttempts: 2, baseDelay: 0.01)
        var attemptCount = 0
        ...
        do {
            ...
            XCTAssertEqual(attemptCount, 2, "Firestore unavailable errors should be retryable")
        }
    }
    
    func testExecute_firestoreDeadlineExceeded_isRetryable() async {
        let policy = RetryPolicy(maxAttempts: 2, baseDelay: 0.01)
        var attemptCount = 0
        ...
        do {
            ...
            XCTAssertEqual(attemptCount, 2, "Firestore deadline exceeded errors should be retryable")
        }
    }
    
    func testExecute_firestoreOtherError_notRetryable() async {
        let policy = RetryPolicy(maxAttempts: 3, baseDelay: 0.01)
        var attemptCount = 0
        ...
        do {
            ...
            XCTAssertEqual(attemptCount, 1, "Non-retryable Firestore errors should not retry")
        }
    }
