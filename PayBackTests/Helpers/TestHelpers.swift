...
/// Builder pattern for creating test expenses with fluent API
class ExpenseBuilder {
    ...
    /// Set the overall settlement status
    @discardableResult
    ...
    /// Create equal splits for the involved members
    @discardableResult
    func withEqualSplits() -> ExpenseBuilder {
        guard !involvedMemberIds.isEmpty else {
            return self
        }
        
        let memberCount = involvedMemberIds.count
        let baseAmount = totalAmount / Double(memberCount)
        let remainder = totalAmount - (baseAmount * Double(memberCount))
        
        // Sort member IDs for deterministic remainder distribution
        let sortedMemberIds = involvedMemberIds.sorted { $0.uuidString < $1.uuidString }
        
        self.splits = sortedMemberIds.enumerated().map { index, memberId in
            // Distribute remainder to first N members
            let extraCents = index < Int(abs(remainder) * 100) ? (remainder > 0 ? 0.01 : -0.01) : 0.0
            let amount = baseAmount + extraCents
