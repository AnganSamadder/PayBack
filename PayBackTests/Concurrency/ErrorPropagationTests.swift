    ...
    func test_wrappedError_preservesUnderlyingError() async {
        ...
        func throwWrappedError() async throws -> String {
            do {
                ...
                throw WrapperError(underlyingError: error)
            }
        }
        ...
        do {
            ...
            XCTFail("Should have thrown error")
        } catch let error as WrapperError {
            XCTAssertTrue(error.underlyingError is LinkingError)
            if let linkingError = error.underlyingError as? LinkingError {
                XCTAssertEqual(linkingError, LinkingError.tokenInvalid)
            }
        } catch {
            XCTFail("Wrong error type: \(error)")
