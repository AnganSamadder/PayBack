...
function analyzeCSV(filePath) {
  ...
  data.sections.GROUP_MEMBERS.forEach((m, idx) => {
    ...
    if (m.length < 3) {
      data.errors.push(
        `GROUP_MEMBERS[${idx}]: Column count mismatch (expected >=3, got ${m.length})`
      );
    }
  });
  ...
  data.sections.EXPENSE_INVOLVED_MEMBERS.forEach((im, idx) => {
    ...
    if (im.length !== 2) {
      data.errors.push(
        `EXPENSE_INVOLVED_MEMBERS[${idx}]: Column count mismatch (expected 2, got ${im.length})`
      );
    }
  });
  ...
  data.sections.EXPENSE_SPLITS.forEach((s, idx) => {
    ...
    if (s.length !== 5) {
      data.errors.push(
        `EXPENSE_SPLITS[${idx}]: Column count mismatch (expected 5, got ${s.length})`
      );
    }
  });
