...
function main() {
  ...
  const passed =
    results.groups.missingInConvex.length === 0 &&
  ...
  if (!passed) {
    ...
    if (results.groups.missingInConvex.length > 0) {
      summary.push(
        `  Missing Group IDs (in CSV but not Convex): ${results.groups.missingInConvex.slice(0, 10).join(", ")}${results.groups.missingInConvex.length > 10 ? "..." : ""}`
      );
    }
    if (results.groups.extraInConvex.length > 0) {
      summary.push(
        `  Extra Group IDs (in Convex but not CSV):   ${results.groups.extraInConvex.slice(0, 10).join(", ")}${results.groups.extraInConvex.length > 10 ? "..." : ""}`
      );
    }
    if (results.expenses.missingInConvex.length > 0) {
      summary.push(
        `  Missing Expense IDs (in CSV but not Convex): ${results.expenses.missingInConvex.slice(0, 10).join(", ")}${results.expenses.missingInConvex.length > 10 ? "..." : ""}`
      );
    }
    if (results.expenses.extraInConvex.length > 0) {
      summary.push(
        `  Extra Expense IDs (in Convex but not CSV):   ${results.expenses.extraInConvex.slice(0, 10).join(", ")}${results.expenses.extraInConvex.length > 10 ? "..." : ""}`
      );
    }
  }
